...
- Status: Closed; implemented. Leading import blocks, trailing export blocks,
  and `static`/`static mut`/direct-Atomic module values replace the old
  declaration-prefixed syntax throughout the parser, checker, and generator.
  Verified by `go test ./...` and the tagged C23 suite compiling and running
  under GCC, Clang, and `zig cc`. `docs/reference.md` documents the grammar,
  import/export rules, and module-value/static-initializer/C23 contracts
- Created: 2026-09-13
- Updated: 2026-09-14
...
- `static` explicitly introduces a module value in every module. An ordinary
  root binding remains executable local storage in generated `main`.
...
top-level-item = declaration-item | module-value | statement ;
module-value = "static" , [ "mut" ] , identifier , [ ":" , type-expression ]
               , ":=" , expression ;
```

Semantic restrictions narrow this shape:

- `module-value` is top-level only and must satisfy Static initialization
  below. It is the only spelling that creates module storage.
- Imported modules still reject executable top-level statements.
- The entrypoint retains executable top-level statements and ordinary
  declarations. Those declarations remain local to generated `main`, cannot be
  exported, and do not acquire program-lifetime storage.
- `import`, `export`, and their matching `end` are block syntax, never
  expressions or local statements.
- `from` is contextual only between an import alias and its path. Existing
  bindings, parameters, fields, and methods named `from` remain valid.
  `module` ceases to be reserved because this RFC removes its only language
  use.
- `static` becomes reserved and has no expression or local-declaration use.
...
- Exporting a type does not export its methods. Each method is listed
  explicitly.
- Exporting an ADT type makes its constructors and variants available through
  that type. Variants are not separate export entries.
...
```hexal
static DefaultPort: Int32 := 8080
static mut RequestCount: UInt64 := 0
...
- Only a `static` declaration is a module value. In the entrypoint,
  `result := run()` remains an executable local initialized when `main` reaches
  it; it is not subject to the static-initializer allowlist.
...
```hexal
static RequestCount: Atomic<UInt64> := Atomic<UInt64>(0)
...
- Bool, integer, Size, Byte, Float, Rune, Strand, and String literals;
- unary `+` or `-` applied directly to a numeric literal when the resulting
  value is valid for its contextual type;
...
- a direct fixed `static Atomic<T>` module value initialized by
...
- operations, conversions, indexing, member reads, and references to another
  module value;
- non-literal String expressions, List, Dict, Task, Channel, Mutex, Stash,
  Pool, IO, File, pointers, Slice, Fun, Error, an aggregate transitively
  containing one, or an aggregate containing Atomic; direct Atomic is governed
...
static mut CurrentDirection: Direction := Direction.North()
```

Neither of these is valid:

```hexal
static Connection := try Database.connect("localhost")
static Names := List<String>(Heap())
```
...
1. Recognize contextual `from` only inside import entries, reserve `static`,
   and remove `module` if repository search confirms it has no other current
   grammar role.
2. Add a top-level-only static module-value node distinct from ordinary root
   declarations; reject `static` in functions and other nested scopes.
3. Replace import declarations and export modifiers with explicit block nodes.
4. Enforce one leading import block and one trailing export block during
   parsing, with exact structural diagnostics.
5. Preserve source tokens for every entry so module and export diagnostics keep
...
1. Admit top-level `static` and `static mut` module values in every reachable
   module. Preserve ordinary root declarations as executable locals and reject
   them in imported modules.
...
- `from` remains valid as an ordinary identifier outside the import-entry
  position; `module` is available as an ordinary identifier after its old use
  is removed.
- `static` is reserved, top-level only, and must introduce a module value.
...
- Exporting an ADT type exposes its constructors and variants through that
  type without separate variant entries.
...
- Only `static` and `static mut` create module values. Ordinary entrypoint
  declarations retain runtime initialization and local `main` storage; an
  imported module rejects an ordinary executable declaration.
...
- Concurrent access to an ordinary mutable module value gains no implicit lock,
  atomic operation, or race diagnostic. Unsynchronized conflicting access from
  multiple Tasks is programmer error and may produce a C data race; this is the
  same explicit manual-synchronization boundary as shared Ptr and writable
  Slice aliases. Atomic or Mutex is the safe shared-mutation mechanism.
