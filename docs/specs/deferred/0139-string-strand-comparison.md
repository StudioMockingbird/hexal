...
- Status: Open Discussion; not scheduled. Design state: Implementation-ready; implementation not started
- Created: 2026-08-27
- Origin: RFC 0103 finding F5
- Coordinates with: `docs/reference.md`
- Amended for RFC 0152 (generic `Strand<N>`): Semantics point 4 below
  generalizes from a hardcoded 32-byte bound to `N`, `N` being whatever
  capacity the specific `Strand<N>` operand carries. This RFC does not gain a
  Strand-vs-Strand cross-capacity comparison path — that is RFC 0152's
  Semantics 11, a plain fixed-length `memcmp` needing no length-aware
  machinery at all. This RFC's own scope stays String-vs-Strand only, since
  `String` (unlike Strand) carries an explicit stored byte length rather than
  a scan bound.
...
4. Strand's logical byte length is the first zero byte or `N` bytes, `N`
   being that operand's own `Strand<N>` capacity (RFC 0152) — 32 total
   bytes / 31 payload bytes was the only capacity when this point was
   written, and is now just the `N = 31` instance. Its canonical
   representation already forbids embedded zero after the content.
...
- A `Strand<N>` of any capacity compares without reading beyond its storage.
