...
- the fundamental alignment guaranteed by `malloc` and `calloc`, which must be
  ...
  Default allocation takes no generated alignment argument, because no current
  Hexal type is over-aligned; a profile that admits an over-aligned
  representation must either supply aligned allocation or reject that
  representation, and cannot rely on a generated assertion to make the fact
  safe.
