...
func TestArrayDeclarationLiteralAndIndexing(t *testing.T) {
	result := compileSource("let fixed: Array<Int32, 3> = [10, 20, 30] let total: Int32 = fixed[0] + fixed[2] let count: Size = fixed.length() let empty: Bool = fixed.length() == 0 let last: Int32 = fixed[2]")
...
func TestArrayMutableElementWrite(t *testing.T) {
	result := compileSource("let mut fixed: Array<Int32, 2> = [1, 2] fixed[0] = 7 fixed[1] = fixed[0] + 1")
	if result.ExitCode != compiler.ExitSuccess {
		t.Fatalf("Compile exit code = %d (%v), want %d", result.ExitCode, result.Stderr, compiler.ExitSuccess)
	}
	for _, want := range []string{
...
func TestArrayConstantIndexOutOfBoundsIsACompileError(t *testing.T) {
	for _, source := range []string{
		"let fixed: Array<Int32, 2> = [1, 2] let bad: Int32 = fixed[2]",
	} {
...
func TestArrayLiteralRequiresExpectedTypeAndExactCount(t *testing.T) {
	for _, testCase := range []struct {
		...
		{"let value: Int32 = [1, 2]", "an array literal requires an expected Array<T, N> destination type"},
		{"let fixed: Array<Int32, 3> = [1, 2]", "Array<Int32, 3> requires exactly 3 elements; got 2"},
		{"let fixed: Array<Int32, 2> = [1, 2, 3]", "Array<Int32, 2> requires exactly 2 elements; got 3"},
		{"let fixed: Array<Int32, 2> = [1, true]", "expected Int32 initializer; got Bool"},
		{"let empty: Array<Int32, 2> = []", "an array literal requires at least one element"},
	} {
...
func TestArrayIndexErrors(t *testing.T) {
	for _, testCase := range []struct {
		...
		{"let fixed: Array<Int32, 2> = [1, 2] let bad: Int32 = fixed[true]", "an array index must be an integer; got Bool"},
		{"let fixed: Array<Int32, 2> = [1, 2] let bad: Int32 = fixed[-1]", "an array index must be non-negative"},
		{"let value: Int32 = 1 let bad: Int32 = value[0]", "cannot index Int32; expected Array<T, N>"},
		{"let fixed: Array<Int32, 2> = [1, 2] let bad: Int32 = fixed.first()", "Array<Int32, 2> has no method first"},
		{"let fixed: Array<Int32, 2> = [1, 2] let bad: UInt64 = fixed.length(1)", "length expects no arguments"},
		{"let fixed: Array<Int32, 2> = [1, 2] let bad: Int32 = fixed.at()", "Array<Int32, 2> has no method at"},
...
func TestNestedArrays(t *testing.T) {
	result := compileSource("let grid: Array<Array<Int32, 2>, 2> = [[1, 2], [3, 4]] let corner: Int32 = grid[1][0]")
	if result.ExitCode != compiler.ExitSuccess {
		t.Fatalf("Compile exit code = %d (%v), want %d", result.ExitCode, result.Stderr, compiler.ExitSuccess)
	}
	for _, want := range []string{
...
func TestArrayTrailingCommaLiteral(t *testing.T) {
	result := compileSource("let fixed: Array<Int32, 3> = [10, 20, 30, ]")
	if result.ExitCode != compiler.ExitSuccess {
		t.Fatalf("Compile exit code = %d (%v), want %d", result.ExitCode, result.Stderr, compiler.ExitSuccess)
	}
