...
func TestLayoutQueriesCompile(t *testing.T) {
	source := "type Node is struct\n    x: Int32,\n    y: Float64,\nend\nfun layout_demo(): Size do\n    let a: Size = size_of<Int32>()\n    let b: Size = align_of<Node>()\n    let c: Size = size_of<String>()\n    let d: Size = size_of<Array<UInt8, 4>>()\n    return a + b + c + d\nend\n"
...
func TestVolatileWriteProducesNoValue(t *testing.T) {
	valid := compileSource("let mut value: Int32 = 1 let slot: Ptr<mut Int32> = @value slot.write_volatile(2)")
	if valid.ExitCode != compiler.ExitSuccess {
		t.Fatalf("write_volatile as a statement = %v", valid.Stderr)
	}
	invalid := compileSource("let mut value: Int32 = 1 let slot: Ptr<mut Int32> = @value let bad: Int32 = slot.write_volatile(2)")
