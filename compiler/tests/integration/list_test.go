...
func TestListViewDerivationAndInvalidation(t *testing.T) {
	result := compileSource("fun demo(h: Heap) do\n    let values: List<Int32> = List<Int32>(h)\n    defer values.free(h)\n    values.push(1)\n    values.push(2)\n    let view: Slice<Int32> = values.slice(0, 2)\n    let total: Int32 = view[0] + view[1]\n    values[0] = 9\nend")
	if result.ExitCode != compiler.ExitSuccess {
		t.Fatalf("Compile exit code = %d (%v), want %d", result.ExitCode, result.Stderr, compiler.ExitSuccess)
	}
	if !strings.Contains(rootC(t, result), "const hex_slice_Int32 hex_v_view = hex_list_slice_Int32(hex_v_values, (size_t)(0), (size_t)(2));") {
		t.Fatalf("modules/app.c = %q, want list slice", rootC(t, result))
	}
}

// Slices are plain descriptors; mutating or freeing the source List while a
// slice is live is the programmer's responsibility.
func TestListViewAfterStructuralMutationIsValid(t *testing.T) {
	for _, source := range []string{
		"fun demo(h: Heap) do\n    let values: List<Int32> = List<Int32>(h)\n    defer values.free(h)\n    let view: Slice<Int32> = values.slice(0, 1)\n    values.push(1)\nend",
		"fun demo(h: Heap) do\n    let values: List<Int32> = List<Int32>(h)\n    defer values.free(h)\n    let view: Slice<Int32> = values.slice(0, 1)\n    let dropped: Int32 = values.pop()\nend",
		"fun demo(h: Heap) do\n    let values: List<Int32> = List<Int32>(h)\n    defer values.free(h)\n    let view: Slice<Int32> = values.slice(0, 1)\n    values.clear()\nend",
		"fun demo(h: Heap) do\n    let values: List<Int32> = List<Int32>(h)\n    let view: Slice<Int32> = values.slice(0, 1)\n    values.free(h)\nend",
	} {
...
func TestListStringMutationAfterReadIsValid(t *testing.T) {
	for _, source := range []string{
		"fun demo(h: Heap) do\n    let names: List<String> = List<String>(h)\n    defer names.free(h)\n    names.push(\"a\")\n    let first: String = names[0]\n    names[0] = \"b\"\nend",
		"fun demo(h: Heap) do\n    let names: List<String> = List<String>(h)\n    defer names.free(h)\n    names.push(\"a\")\n    let first: String = names[0]\n    let dropped: String = names.pop()\nend",
		"fun demo(h: Heap) do\n    let names: List<String> = List<String>(h)\n    defer names.free(h)\n    names.push(\"a\")\n    let first: String = names[0]\n    names.clear()\nend",
		"fun demo(h: Heap) do\n    let names: List<String> = List<String>(h)\n    names.push(\"a\")\n    let first: String = names[0]\n    names.free(h)\nend",
		"fun demo(h: Heap) do\n    let mut names: List<String> = List<String>(h)\n    defer names.free(h)\n    names.push(\"a\")\n    let first: String = names[0]\n    names = List<String>(h)\nend",
...
func TestListRestrictions(t *testing.T) {
	...
	if result := compileSource("type Holder is struct values: List<Int32>, end\nfun demo(h: Heap) do\n    let holder: Holder = Holder(values = List<Int32>(h))\n    holder.values.push(1)\nend"); result.ExitCode != compiler.ExitSuccess {
