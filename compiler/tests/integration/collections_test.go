...
func TestStorabilityRule(t *testing.T) {
	accepted := []string{
		...
		"views: List<Slice<Int32>> := List<Slice<Int32>>(Heap())\n",
		"lookup: Dict<Strand, List<Int32>> := Dict<Strand, List<Int32>>(Heap())\n",
		"value: String | Nil := nil\n",
		"value: List<Int32> | Nil := nil\n",
		"value: Slice<Int32> | Nil := nil\n",
		"v: Slice<Slice<Int32>> := Slice<Slice<Int32>>.empty()\n",
		"v: Slice<String> := Slice<String>.empty()\n",
		"fun s(xs: List<String>): Slice<String> do\n    return xs.slice(0, 1)\nend\n",
