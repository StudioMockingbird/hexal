...
func TestCoreScalars(t *testing.T) {
	source := "let visible: Bool = true " +
		"let u8: UInt8 = 255 let u16: UInt16 = 65535 " +
		"let u32: UInt32 = 4294967295 let u64: UInt64 = 18446744073709551615 " +
		"let i8: Int8 = -128 let i16: Int16 = -32768 " +
		"let i32: Int32 = -2147483648 let i64: Int64 = -9223372036854775808 " +
		"let f32: Float32 = -0.0 let f64: Float64 = 6.02e23"
...
func TestIntegerRadices(t *testing.T) {
	result := compileSource("let decimal: UInt16 = 255 let hexadecimal: UInt16 = 0xFF let binary: UInt16 = 0b1111_1111 let octal: UInt16 = 0o377")
	if result.ExitCode != compiler.ExitSuccess {
		t.Fatalf("Compile failed: %#v", result.Stderr)
	}
	for _, want := range []string{
...
func TestContextualAssignmentAndPointerValue(t *testing.T) {
	result := compileSource("let mut byte: UInt8 = 0 byte = 255 let mut value: Int8 = 0 let writer: Ptr<mut Int8> = @value ^writer = -128")
	if result.ExitCode != compiler.ExitSuccess {
		t.Fatalf("Compile failed: %#v", result.Stderr)
	}
...
func TestRejectsScalarMismatchesAndRanges(t *testing.T) {
	testCases := []struct {
		...
		{"let x: UInt8 = 256", "outside the UInt8 range"},
		{"let x: Int8 = 128", "outside the Int8 range"},
		{"let x: Int8 = -129", "outside the Int8 range"},
		{"let x: UInt64 = 18446744073709551616", "outside the UInt64 range"},
		{"let x: UInt8 = -0", "negated integer literal requires a signed destination"},
		{"let x: Float32 = 1", "expected Float32 initializer; got Int32"},
		{"let x: Int32 = 1.0", "expected Int32 initializer; got Float64"},
		{"let x: Bool = 1", "expected Bool initializer; got Int32"},
	}
...
func TestGeneralNegativeSyntax(t *testing.T) {
	result := compileSource("let mut name: Int32 = 5 let negative: Int32 = -name let repeated: Int32 = - -name")
	...
	result = compileSource("let x: Int32 = - -- a comment\n 128")
...
func TestFloatUnaryNegation(t *testing.T) {
	result := compileSource("let mut f32Value: Float32 = 1.5 let negative32: Float32 = -f32Value let mut f64Value: Float64 = 2.5 let negative64: Float64 = -f64Value")
	if result.ExitCode != compiler.ExitSuccess || len(result.Stderr) != 0 {
...
func TestNegativeZeroUnaryFolding(t *testing.T) {
	result := compileSource("let f32: Float32 = -(0.0) let f64: Float64 = -(0.0)")
	if result.ExitCode != compiler.ExitSuccess || len(result.Stderr) != 0 {
...
func TestFloatRoundingAndUnderflow(t *testing.T) {
	result := compileSource("let tie: Float32 = 1.000000059604644775390625 let underflow: Float32 = 1e-1000 let negative: Float32 = -1e-1000")
	if result.ExitCode != compiler.ExitSuccess {
		t.Fatalf("Compile failed: %#v", result.Stderr)
	}
	for _, want := range []string{
	...
	for _, source := range []string{"let x: Float32 = 1e1000", "let x: Float64 = 1e10000"} {
		result = compileSource(source)
...
func TestPointerScalarMappings(t *testing.T) {
	result := compileSource("let mut value: UInt8 = 1 let reader: Ptr<UInt8> = @value let writer: Ptr<mut UInt8> = @value let mut float_value: Float32 = 1.0 let float_writer: Ptr<mut Float32> = @float_value let nested: Ptr<Ptr<mut Float32>> = @float_writer")
	if result.ExitCode != compiler.ExitSuccess {
		t.Fatalf("Compile failed: %#v", result.Stderr)
	}
	for _, want := range []string{
...
func TestSizeLiteralTargetGuard(t *testing.T) {
	result := compileSource("let count: Size = 5000000000\nlet small: Size = 3\n")
	if result.ExitCode != compiler.ExitSuccess {
...
func TestBoolDeclaration(t *testing.T) {
	result := compileSource("let flag: Bool = true")
	if result.ExitCode != compiler.ExitSuccess {
		t.Fatalf("Compile exit code = %d (%v), want %d", result.ExitCode, result.Stderr, compiler.ExitSuccess)
	}
...
func TestByteAndByteTypeBehavior(t *testing.T) {
	result := compileSource("let letter: UInt8 = b'A'")
	if result.ExitCode != compiler.ExitSuccess {
	...
	result = compileSource("let letter: Byte = 65 let again: UInt8 = letter")
	if result.ExitCode != compiler.ExitSuccess {
...
func TestAdditionalNumericTypes(t *testing.T) {
	result := compileSource("let count: Int64 = 9_000_000_000 let single: Float32 = 3.14 let precise: Float64 = 6.02e23")
	if result.ExitCode != compiler.ExitSuccess {
		t.Fatalf("Compile exit code = %d (%v), want %d", result.ExitCode, result.Stderr, compiler.ExitSuccess)
	}
	for _, want := range []string{
...
func TestDefaultsUncontextualizedNumericFamilies(t *testing.T) {
	...
	result = compileSource("let whole: Int32 = 13 let fraction: Float64 = 3.14")
...
func TestHexLiteralPreservesSpelling(t *testing.T) {
	result := compileSource("let mask: Int32 = 0xFF")
	if result.ExitCode != compiler.ExitSuccess {
		t.Fatalf("Compile exit code = %d (%v), want %d", result.ExitCode, result.Stderr, compiler.ExitSuccess)
	}
