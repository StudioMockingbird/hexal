...
const inferRejection = "`let` requires an initializer whose type does not depend on context"
...
func TestInferredDeclarationTakesTheInitializersType(t *testing.T) {
	for _, testCase := range []struct{ name, source, want string }{
		...
		},
		{
			...
			"fun compute(): Int64 do\n    return 7\nend\nfun demo(ready: Bool) do\n    let x = match ready | true then compute() | false then compute() end\nend",
			"int64_t hex_v_x",
		},
	} {
...
func TestInferredDeclarationRejectsMatchOverBareLiterals(t *testing.T) {
	assertRejects(t, "fun demo(ready: Bool) do\n    let x = match ready | true then 1 | false then 0 end\nend", inferRejection)
	...
	assertRejects(t,
		"fun compute(): Int64 do\n    return 7\nend\nfun demo(ready: Bool) do\n    let x = match ready | true then compute() | false then 0 end\nend",
		"match arm result types do not agree")
}

// `:` and `=` are independent tokens, so the spaced form `x : = 5` is the
// removed `:=` declaration operator and receives its migration diagnostic.
func TestSpacedColonEqualReportsTheRemovedOperator(t *testing.T) {
	assertRejects(t, "fun demo() do\n    x : = 5\nend", "':=' is not a declaration operator; use 'let name = value'")
}
