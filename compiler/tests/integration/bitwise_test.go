...
func TestBitwiseOperators(t *testing.T) {
	result := compileSource("fun demo() do\n    let masked: UInt32 = 0xFFFF0000 & 0x0000FFFF\n    let xor: UInt32 = 0xFF00 ^ 0x0FF0\n    let combined: UInt8 = 0xF0 | 0x0F\n    let complement: UInt8 = ~0x0F\n    let small: UInt8 = 0xF0\n    let wide: UInt16 = 0x0F0F\n    let widened: UInt16 = small & wide\n    let flags: UInt32 = 0xFFFF\n    let result: UInt32 = flags & 0x00FF\nend")
...
func TestBitwiseSignedDirectCast(t *testing.T) {
	result := compileSource("fun demo() do\n    let mask: Int8 = ~0\n    let low: Int8 = 0x0F\n    let signed: Int8 = mask & low\n    let negative: Int32 = -1\n    let bits: UInt32 = 0x80000000\n    let cross: Int32 = negative & 0x7FFFFFFF\nend")
	if result.ExitCode != compiler.ExitSuccess {
		t.Fatalf("Compile exit code = %d (%v), want %d", result.ExitCode, result.Stderr, compiler.ExitSuccess)
	}
	for _, want := range []string{
...
func TestShiftInt64SignFillMaskUsesExactWidth(t *testing.T) {
	result := compileSource("fun demo() do\n    let mut negative64: Int64 = -8\n    let halved64: Int64 = negative64 >> 1\n    let sign64: Int64 = negative64 >> 63\n    let mut negative32: Int32 = -8\n    let halved32: Int32 = negative32 >> 1\n    let mut negative8: Int8 = -8\n    let halved8: Int8 = negative8 >> 7\nend")
	if result.ExitCode != compiler.ExitSuccess {
		t.Fatalf("Compile exit code = %d (%v), want %d", result.ExitCode, result.Stderr, compiler.ExitSuccess)
	}
	for _, want := range []string{
...
func TestBitCast(t *testing.T) {
	result := compileSource("fun demo() do\n    let floating: Float64 = 1.0\n    let bits: UInt64 = floating.bit_cast<UInt64>()\n    let again: Float64 = bits.bit_cast<Float64>()\n    let signed: Int32 = -1\n    let unsigned: UInt32 = signed.bit_cast<UInt32>()\n    let back: Int32 = unsigned.bit_cast<Int32>()\n    let narrow: Float32 = 1.5\n    let narrow_bits: UInt32 = narrow.bit_cast<UInt32>()\nend")
	if result.ExitCode != compiler.ExitSuccess {
		t.Fatalf("Compile exit code = %d (%v), want %d", result.ExitCode, result.Stderr, compiler.ExitSuccess)
	}
	for _, want := range []string{
...
func TestNestedGenericClosersStillParse(t *testing.T) {
	result := compileSource("type Link<T> is struct value: T, mut next: Ptr<mut Link<T>> | Nil end let link: Link<Int32> = Link<Int32>(value = 1, next = nil) fun demo() do\n    let pointer: Ptr<Ptr<Int32>> | Nil = nil\n    let inner: Ptr<Int32> | Nil = nil\n    let outer: Ptr<Ptr<Int32>> | Nil = nil\nend")
	if result.ExitCode != compiler.ExitSuccess {
		t.Fatalf("Compile exit code = %d (%v), want %d", result.ExitCode, result.Stderr, compiler.ExitSuccess)
	}
}
