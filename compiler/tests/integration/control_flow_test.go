...
func TestConditionalAndLoopLowering(t *testing.T) {
	result := compileSource("let mut count: Int32 = 0 let mut ready: Bool = true if ready then let mut local: Int32 = 1 count = count + local elseif false then count = 9 else count = 8 end while count < 3 do count = count + 1 if count == 2 then continue end if count > 2 then break end end")
...
func TestControlFlowScopesAndShadowing(t *testing.T) {
	result := compileSource("let mut value: Int32 = 0 if true then value = 1 else let value: Int32 = 2 end if false then let value: Int32 = 3 end")
	if result.ExitCode != compiler.ExitSuccess || len(result.Stderr) != 0 {
	...
	invalid := compileSource("if true then let local: Int32 = 1 end let read: Int32 = local")
...
func TestControlFlowDefiniteReturns(t *testing.T) {
	valid := compileSource("fun classify(value: Int32): Int32 do if value > 0 then return 1 elseif value == 0 then return 0 else return -1 end end let result: Int32 = classify(1)")
...
func TestControlFlowNestedLoopsAndLineMappings(t *testing.T) {
	result := compileSource("let mut outer: Int32 = 0\nwhile outer < 2 do\n    let mut inner: Int32 = 0\n    while inner < 2 do\n        inner = inner + 1\n        if inner == 1 then\n            continue\n        end\n        break\n    end\n    outer = outer + 1\n    break\nend")
	if result.ExitCode != compiler.ExitSuccess || len(result.Stderr) != 0 {
...
func TestControlFlowLoopScopeCleanup(t *testing.T) {
	result := compileSource("while false do let local: Int32 = 1 end let read: Int32 = local")
...
func TestControlFlowReturnDiagnosticsDoNotMaskChildErrors(t *testing.T) {
	...
	if parsedInvalid.ExitCode != compiler.ExitFailure || !strings.Contains(parsedMessage, "expected '=' for an assignment") || strings.Contains(parsedMessage, "may fall through without returning") {
	...
	if unrelatedParserError.ExitCode != compiler.ExitFailure || !strings.Contains(unrelatedMessage, "expected '=' for an assignment") || strings.Contains(unrelatedMessage, "may fall through without returning") {
	...
	if dottedCallRecovery.ExitCode != compiler.ExitFailure || !strings.Contains(dottedMessage, "expected '=' for an assignment") || strings.Contains(dottedMessage, "unknown variable point") {
		t.Fatalf("dotted call sibling recovery = %#v", dottedCallRecovery)
	}
}

func TestMethodControlFlowLowering(t *testing.T) {
	result := compileSource("type Counter is struct mut count: Int32 end method Counter.step(amount: Int32): Int32 do if amount > 0 then let mut next: Counter = self next.count = next.count + amount return next.count else return 0 end end let mut counter: Counter = Counter(count = 1) let result: Int32 = counter.step(2)")
	if result.ExitCode != compiler.ExitSuccess || len(result.Stderr) != 0 {
