...
func TestGenericAliasSpecializesTransparently(t *testing.T) {
	result := compileSource("type Pointer<T> is Ptr<T> let mut value: Int32 = 1 let pointer: Pointer<Int32> = @value")
...
func TestGenericObjectSpecializesWithSubstitutedMembers(t *testing.T) {
	result := compileSource("type Box<T> is struct value: T end let box: Box<Int32> = Box<Int32>(value = 42)")
	if result.ExitCode != compiler.ExitSuccess {
		t.Fatalf("Compile exit code = %d (%v), want %d", result.ExitCode, result.Stderr, compiler.ExitSuccess)
	}
...
func TestGenericFunctionCallAndExplicitArguments(t *testing.T) {
	result := compileSource("fun identity<T>(value: T): T do\nreturn value\nend let first: Int32 = identity(42) let second: Int64 = identity<Int64>(42)")
	if result.ExitCode != compiler.ExitSuccess {
		t.Fatalf("Compile exit code = %d (%v), want %d", result.ExitCode, result.Stderr, compiler.ExitSuccess)
	}
...
func TestGenericObjectLiteralInfersFromExpectedType(t *testing.T) {
	result := compileSource("type Box<T> is struct value: T end let box: Box<Int32> = Box(value = 42)")
	if result.ExitCode != compiler.ExitSuccess {
		t.Fatalf("Compile exit code = %d (%v), want %d", result.ExitCode, result.Stderr, compiler.ExitSuccess)
	}
...
func TestGenericNestedSpecializationsReuseOneCName(t *testing.T) {
	result := compileSource("fun identity<T>(value: T): T do\nreturn value\nend let first: Int32 = identity(1) let second: Int32 = identity(2)")
	if result.ExitCode != compiler.ExitSuccess {
		t.Fatalf("Compile exit code = %d (%v), want %d", result.ExitCode, result.Stderr, compiler.ExitSuccess)
	}
...
func TestGenericPointerIndirectedRecursionIsFinite(t *testing.T) {
	result := compileSource("type Link<T> is struct value: T, mut next: Ptr<mut Link<T>> | Nil, end let link: Link<Int32> = Link<Int32>(value = 1, next = nil)")
	if result.ExitCode != compiler.ExitSuccess {
		t.Fatalf("Compile exit code = %d (%v), want %d", result.ExitCode, result.Stderr, compiler.ExitSuccess)
	}
