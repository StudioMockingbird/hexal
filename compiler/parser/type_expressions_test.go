...
func TestParseBuiltinNilAndUnknownTypeExpressions(t *testing.T) {
	if got := fmt.Sprintf("%T", parseAnnotation(t, "let nothing: Nil = nil")); got != "parser.NilTypeExpression" {
		t.Fatalf("Nil annotation type = %q, want parser.NilTypeExpression", got)
	}
	if got := fmt.Sprintf("%T", parseAnnotation(t, "let opaque: Unknown = value")); got != "parser.UnknownTypeExpression" {
		t.Fatalf("Unknown annotation type = %q, want parser.UnknownTypeExpression", got)
	}

	pointer, ok := parseAnnotation(t, "let erased: Ptr<Unknown> = value").(PtrTypeExpression)
	if !ok {
...
func TestParseNullableTypeExpressionPreservesNestedAndRepeatedSuffixes(t *testing.T) {
	typeExpression := parseAnnotation(t, "let nested: Ptr<mut Ptr<mut Node> | Nil> | Nil | Nil = nil")
	...
	if got := fmt.Sprintf("%T", parseInitializer(t, "let nothing: Nil = nil")); got != "parser.NilLiteral" {
		t.Fatalf("nil initializer type = %q, want parser.NilLiteral", got)
	}
}

func TestParseGeneralUnionTypeExpression(t *testing.T) {
	typeExpression := parseAnnotation(t, "let value: (Int32 | Float64) | Nil = nil")
...
func TestParseTypeTestExpression(t *testing.T) {
	typeExpression := parseInitializer(t, "let tested: Bool = value is Int32")
...
func TestParseTypeTestPrecedence(t *testing.T) {
	message := parseError(t, "let tested: Bool = value is Int32 == expected")
	...
	initializer := parseInitializer(t, "let tested: Bool = (value is Int32) == expected")
...
func TestParseRejectsChainedTypeTests(t *testing.T) {
	message := parseError(t, "let tested: Bool = value is Int32 is Bool")
...
func TestParseGeneralUnionAcceptsAnyMemberOrder(t *testing.T) {
	for _, source := range []string{
		"let value: Nil | Ptr<Int32> = nil",
		"let value: Ptr<Int32> | Ptr<mut Int32> = nil",
	} {
...
func TestParseRejectsObjectTypeOutsideTypeDeclaration(t *testing.T) {
	for _, source := range []string{
		"point: { let x: Int32 } = value",
		"point: { let x: Int32 } | Nil = value",
...
func TestParseMutPtrTypeExpression(t *testing.T) {
	for _, testCase := range []struct {
		...
		{"let x: Ptr<mut Int32> = y", true},
		{"let x: Ptr<mut Ptr<mut Int32>> = y", true},
...
func TestParseSliceTypeExpression(t *testing.T) {
	for _, testCase := range []struct {
		...
		{"let x: Slice<Int32> = y", false},
		{"let x: Slice<mut Int32> = y", true},
...
func TestParseFunctionTypeExpression(t *testing.T) {
	function, ok := parseAnnotation(t, "let handler: Fun<(Int32, Bool) : Int32> = f").(FunctionTypeExpression)
	if !ok {
...
func TestParseFunctionTypeWithoutReturn(t *testing.T) {
	function := parseAnnotation(t, "let handler: Fun<(Int32)> = f").(FunctionTypeExpression)
...
func TestParseFunctionTypeWithoutParameters(t *testing.T) {
	function := parseAnnotation(t, "let handler: Fun<()> = f").(FunctionTypeExpression)
...
func TestParseNestedFunctionType(t *testing.T) {
	function := parseAnnotation(t, "let handler: Fun<(Fun<(Int32) : Int32>) : Int32> = f").(FunctionTypeExpression)
