...
func TestPointerOrderingRejected(t *testing.T) {
	for _, operator := range []string{"<", "<=", ">", ">="} {
		...
		if result.ExitCode != ExitFailure || len(result.Stderr) == 0 || !strings.Contains(result.Stderr[0], "ordering is unavailable for Ptr<Int32>") {
...
func TestPointerIndexingRejected(t *testing.T) {
	for _, testCase := range []struct {
		...
		{"read", "fun demo()\n    value: Int32 = 1\n    pointer: Ptr<Int32> = ref value\n    bad: Int32 = pointer[0]\nend"},
		{"write", "fun demo()\n    mut value: Int32 = 1\n    pointer: MutPtr<Int32> = ref value\n    pointer[0] = 5\nend"},
		{"nullable narrowed", "fun demo()\n    value: Int32 = 1\n    mut pointer: Ptr<Int32> | Nil = ref value\n    if pointer != nil\n        bad: Int32 = pointer[0]\n    end\nend"},
	} {
		t.Run(testCase.name, func(t *testing.T) {
			result := Compile(testCase.source)
			if result.ExitCode != ExitFailure || len(result.Stderr) == 0 || !strings.Contains(result.Stderr[0], "cannot index") {
...
func TestPointerIntegerConversionsRejected(t *testing.T) {
	result := Compile("fun demo()\n    value: Int32 = 1\n    pointer: Ptr<Int32> = ref value\n    bad: UInt64 = pointer\nend")
	if result.ExitCode != ExitFailure || len(result.Stderr) == 0 || !strings.Contains(result.Stderr[0], "expected UInt64 initializer, got Ptr<Int32>") {
		t.Fatalf("Compile stderr = %#v, want pointer-to-integer rejection", result.Stderr)
	}
	result = Compile("fun demo()\n    address: UInt64 = 42\n    bad: Ptr<Int32> = address\nend")
	if result.ExitCode != ExitFailure || len(result.Stderr) == 0 || !strings.Contains(result.Stderr[0], "expected Ptr<Int32> initializer, got UInt64") {
