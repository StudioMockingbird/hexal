...
func checkBitCastCall(call parser.CallExpression, callee parser.PropertyExpression, receiver checkedExpression, environment *scope, typeEnvironment *compilerTypes.Environment) checkedExpression {
	if len(call.TypeArguments) != 1 {
		return checkedExpression{token: callee.Property, diagnostic: &compilerTypes.Diagnostic{
			...
			Message:  "bit_cast requires exactly 1 explicit type argument",
		}}
	}
	if len(call.Arguments) != 0 {
		return checkedExpression{token: callee.Property, diagnostic: &compilerTypes.Diagnostic{
			...
			Line:     callee.Property.Line,
			Column:   callee.Property.Column,
			Message:  "bit_cast accepts no value arguments",
		}}
	}
	...
	if !bitCastEligibleType(receiver.typ) || !bitCastEligibleType(target) {
		return checkedExpression{token: callee.Property, diagnostic: &compilerTypes.Diagnostic{
			...
			Line:     callee.Property.Line,
			Column:   callee.Property.Column,
			Message:  "bit_cast requires equal-width eligible scalar types; got " + receiver.typ.Name + " and " + target.Name,
		}}
	}
	if receiver.typ.Bits != target.Bits {
		return checkedExpression{token: callee.Property, diagnostic: &compilerTypes.Diagnostic{
			...
			Line:     callee.Property.Line,
			Column:   callee.Property.Column,
			Message:  fmt.Sprintf("bit_cast requires equal-width eligible scalar types; got %s and %s", receiver.typ.Name, target.Name),
		}}
	}
...
func checkEndianToBytesCall(call parser.CallExpression, callee parser.PropertyExpression, receiver checkedExpression, environment *scope, typeEnvironment *compilerTypes.Environment) checkedExpression {
	if len(call.Arguments) != 0 || len(call.TypeArguments) != 0 {
		return checkedExpression{token: callee.Property, diagnostic: &compilerTypes.Diagnostic{
			...
			Line:     callee.Property.Line,
			Column:   callee.Property.Column,
			Message:  callee.Property.Lexeme + " takes no arguments",
		}}
	}
	if !endianEligibleType(receiver.typ) {
		return checkedExpression{token: callee.Property, diagnostic: &compilerTypes.Diagnostic{
			...
			Line:     callee.Property.Line,
			Column:   callee.Property.Column,
			Message:  callee.Property.Lexeme + " requires a fixed-width integer receiver; got " + receiver.typ.Name,
		}}
	}
	...
	if array == (compilerTypes.Type{}) {
		return checkedExpression{token: callee.Property, diagnostic: &compilerTypes.Diagnostic{
			...
			Line:     callee.Property.Line,
			Column:   callee.Property.Column,
			Message:  "could not construct the endian byte array type",
		}}
	}
	memberIndex := 0
...
func checkEndianFromBytesCall(call parser.CallExpression, callee lexer.Token, typeEnvironment *compilerTypes.Environment, environment *scope) checkedExpression {
	...
	if !ok || !endianEligibleType(integerType) {
		return checkedExpression{token: property, diagnostic: &compilerTypes.Diagnostic{
			...
			Message:  callee.Lexeme + " has no such operation; from_le_bytes and from_be_bytes require a fixed-width integer type",
		}}
	}
	if len(call.Arguments) != 1 || len(call.TypeArguments) != 0 {
		return checkedExpression{token: property, diagnostic: &compilerTypes.Diagnostic{
			...
			Line:     property.Line,
			Column:   property.Column,
			Message:  property.Lexeme + " expects exactly 1 argument",
		}}
	}
	...
	bytes := checkInitializer(call.Arguments[0], compilerTypes.NewTypeUse(array), tokenOf(call.Arguments[0]), environment, typeEnvironment)
	...
	if !compilerTypes.Equal(bytes.typ, array) {
		return checkedExpression{token: bytes.token, diagnostic: &compilerTypes.Diagnostic{
			Category: compilerTypes.TypeError,
			Stage:    "checker",
			Line:     bytes.token.Line,
			Column:   bytes.token.Column,
			Message:  fmt.Sprintf("%s.from_%s expects Array<Byte, %d>; got %s", callee.Lexeme, endianOrderName(property.Lexeme), integerType.Bits/8, bytes.typ.Name),
		}}
	}
	memberIndex := 0
