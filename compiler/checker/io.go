...
// standard handle.
func checkIOTypeCall(call parser.CallExpression, token parser.VariableExpression, names *scope, typeEnvironment *compilerTypes.Environment) checkedExpression {
	...
	resultUnion := typeEnvironment.UnionType([]compilerTypes.Type{compilerTypes.IOType, compilerTypes.ErrorType})
...
func checkBytesTypeCall(call parser.CallExpression, token parser.VariableExpression, names *scope, typeEnvironment *compilerTypes.Environment) checkedExpression {
	property := call.Callee.(parser.PropertyExpression).Property
	...
	byteList := typeEnvironment.ListType(compilerTypes.UInt8)
	...
	buffer := checkInitializer(call.Arguments[0], compilerTypes.NewTypeUse(byteList), tokenOf(call.Arguments[0]), names, typeEnvironment)
	...
	if names.flow != nil {
		...
		if listID := receiverVariableBinding(buffer.source); listID != 0 && names.flow.sourceReleased(listID) {
...
func checkIOStreamMethodCall(call parser.CallExpression, callee parser.PropertyExpression, receiver checkedExpression, names *scope, typeEnvironment *compilerTypes.Environment) checkedExpression {
	...
	if names.cleanupDepth > 0 && name != "close" {
	...
	if diagnostic := streamClosedDiagnostic(receiver.source, callee.Property, names.flow); diagnostic != nil {
		return checkedExpression{token: callee.Property, diagnostic: diagnostic}
	}
	if name == "read" || name == "write" {
		if diagnostic := streamCapabilityMismatch(name, receiver.source, callee.Property, names.flow); diagnostic != nil {
			return checkedExpression{token: callee.Property, diagnostic: diagnostic}
		}
	}
	...
	switch name {
	case "read":
		checked, diagnostics := checkStreamReadArguments(call, callee, names, typeEnvironment)
		if len(diagnostics) > 0 {
			return checkedExpression{token: callee.Property, diagnostics: diagnostics, diagnostic: &diagnostics[0]}
		}
		arguments = checked
	case "write":
		checked, diagnostics := checkStreamWriteArguments(call, callee, names, typeEnvironment)
		if len(diagnostics) > 0 {
			return checkedExpression{token: callee.Property, diagnostics: diagnostics, diagnostic: &diagnostics[0]}
		}
		arguments = checked
	case "seek":
		checked, diagnostics := checkStreamSeekArguments(call, callee, names, typeEnvironment)
		if len(diagnostics) > 0 {
			return checkedExpression{token: callee.Property, diagnostics: diagnostics, diagnostic: &diagnostics[0]}
		}
		arguments = checked
	...
	resultUnion := streamResultUnion(name, typeEnvironment)
	if resultUnion == (compilerTypes.Type{}) {
	...
	if name == "close" && names.cleanupDepth == 0 {
		...
		markStreamClosed(receiver.source, callee.Property, names.flow)
	}
	return checked
}
...
// bindings.
func checkBytesStreamMethodCall(call parser.CallExpression, callee parser.PropertyExpression, receiver checkedExpression, names *scope, typeEnvironment *compilerTypes.Environment) checkedExpression {
	name := callee.Property.Lexeme
	...
	target := typeEnvironment.MutPtrType(compilerTypes.BytesType)
	...
	if diagnostic := borrowedSourceDiagnostic(adapted, callee.Property, names.flow); diagnostic != nil {
		return checkedExpression{token: callee.Property, diagnostic: diagnostic}
	}
	var arguments []Operand
	switch name {
...
func checkStreamReadArguments(call parser.CallExpression, callee parser.PropertyExpression, names *scope, typeEnvironment *compilerTypes.Environment) ([]Operand, compilerTypes.Diagnostics) {
	byteList := typeEnvironment.ListType(compilerTypes.UInt8)
	...
	into := checkInitializer(call.Arguments[0], compilerTypes.NewTypeUse(byteList), tokenOf(call.Arguments[0]), names, typeEnvironment)
	...
	maximum := checkInitializer(call.Arguments[1], compilerTypes.NewTypeUse(compilerTypes.SizeType), tokenOf(call.Arguments[1]), names, typeEnvironment)
...
func checkStreamWriteArguments(call parser.CallExpression, callee parser.PropertyExpression, names *scope, typeEnvironment *compilerTypes.Environment) ([]Operand, compilerTypes.Diagnostics) {
	byteView := typeEnvironment.ViewType(compilerTypes.UInt8)
	...
	from := checkInitializer(call.Arguments[0], compilerTypes.NewTypeUse(byteView), tokenOf(call.Arguments[0]), names, typeEnvironment)
...
func checkStreamSeekArguments(call parser.CallExpression, callee parser.PropertyExpression, names *scope, typeEnvironment *compilerTypes.Environment) ([]Operand, compilerTypes.Diagnostics) {
	...
	to := checkInitializer(call.Arguments[0], compilerTypes.NewTypeUse(compilerTypes.SeekType), tokenOf(call.Arguments[0]), names, typeEnvironment)
