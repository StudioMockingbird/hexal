...
func TestCheckGenericObjectTypeDeclarationAndUse(t *testing.T) {
	checked, err := Check(parseProgram(t, "type Box<T> is struct value: T end let box: Box<Int32> = Box<Int32>(value = 42)"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestCheckGenericAliasSpecializesTransparently(t *testing.T) {
	_, err := Check(parseProgram(t, "type Pointer<T> is Ptr<T> let pointer: Pointer<Int32> = nil"))
	if err == nil {
...
func TestCheckGenericTypeArityMismatch(t *testing.T) {
	requireDiagnostic(t,
		"type Pair<Left, Right> is struct left: Left, right: Right end let bad: Pair<Int32> = value",
		"generic type Pair expects 2 type arguments; got 1")
}

func TestCheckGenericTypeUnknownArgument(t *testing.T) {
	requireDiagnostic(t,
		"type Box<T> is struct value: T end let bad: Box<Missing> = value",
		"unknown type Missing")
}

func TestCheckGenericObjectPointerIndirectedRecursion(t *testing.T) {
	checked, err := Check(parseProgram(t, "type Link<T> is struct value: T, mut next: Ptr<mut Link<T>> | Nil, end let link: Link<Int32> = Link<Int32>(value = 1, next = nil)"))
	if err != nil {
		t.Fatal(err)
	}
	found := false
...
func TestCheckGenericFunctionExplicitArguments(t *testing.T) {
	checked, err := Check(parseProgram(t, "fun identity<T>(value: T): T do\nreturn value\nend let answer: Int64 = identity<Int64>(42)"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestCheckGenericFunctionValueReferenceInfersFromTarget(t *testing.T) {
	checked, err := Check(parseProgram(t, "fun identity<T>(value: T): T do\nreturn value\nend let callback: Fun<(Int32) : Int32> = identity"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestCheckGenericObjectLiteralInfersFromExpectedType(t *testing.T) {
	checked, err := Check(parseProgram(t, "type Box<T> is struct value: T end let box: Box<Int32> = Box(value = 42)"))
	if err != nil {
		t.Fatal(err)
	}
	found := false
