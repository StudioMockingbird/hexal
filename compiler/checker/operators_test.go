...
func TestCheckContextualTypingThroughOperators(t *testing.T) {
	checked, err := Check(parseProgram(t, "let total: Int64 = 5_000_000_000 + 1 let ratio: Float32 = 1.5 * 2.0"))
	if err != nil {
...
func TestCheckContextualTypingUsesOnlyAdmissibleExpectedTypes(t *testing.T) {
	_, err := Check(parseProgram(t, "let big: Bool = 5_000_000_000 > 1"))
	...
	checked, err := Check(parseProgram(t, "let mut threshold: Int64 = 5_000_000_000 let big: Bool = threshold > 1"))
	if err != nil {
...
func TestCheckContextualTypingAllowsLosslessResultWidening(t *testing.T) {
	...
	// result losslessly.
	checked, err := Check(parseProgram(t, "let a: Int32 = 2 let doubled: Int64 = a + a"))
	if err != nil {
...
func TestCheckOperatorDomains(t *testing.T) {
	for _, testCase := range []struct {
		...
		{"let value: Float64 = 1.0 % 2.0", "operator % requires integer operands; got Float64"},
		{"let left: Bool = true let right: Bool = false let bad: Bool = left < right", "ordering is unavailable for Bool values"},
		{"let count: UInt32 = 5 let bad: Int32 = -count", "negation requires a signed type; got UInt32"},
	} {
...
func TestCheckOperatorSelectsLosslessCommonArithmeticType(t *testing.T) {
	...
	// common type.
	checked, err := Check(parseProgram(t, "let small: Int16 = 1 let large: Int32 = 2 let total: Int32 = small + large"))
	if err != nil {
...
func TestCheckFoldsLiteralArithmeticAndWrapsOverflow(t *testing.T) {
	...
	// literal spellings.
	checked, err := Check(parseProgram(t, "let next: UInt8 = 200 + 1"))
	if err != nil {
		t.Fatalf("Check returned an error: %v", err)
	}
	...
	checked, err = Check(parseProgram(t, "let over: UInt8 = 200 + 100"))
	if err != nil {
		t.Fatalf("Check returned an error: %v", err)
	}
...
func TestCheckKeepsMutableOperationsForRuntimeWrapping(t *testing.T) {
	checked, err := Check(parseProgram(t, "let mut count: UInt8 = 200 let next: UInt8 = count + 100"))
	if err != nil {
		t.Fatalf("Check returned an error: %v", err)
	}
...
func TestCheckFoldsIntegerBooleanAndComparisonOperations(t *testing.T) {
	checked, err := Check(parseProgram(t, "let sum: Int32 = 2 + 3 let difference: Int32 = 9 - 4 let product: Int32 = 3 * 4 let quotient: Int32 = 9 / 2 let remainder: Int32 = 9 % 4 let inverted: Bool = !true let equal: Bool = 2 == 2 let ordered: Bool = 1 < 2 let both: Bool = true and false let either: Bool = false or true"))
	if err != nil {
		t.Fatalf("Check returned an error: %v", err)
	}
...
func TestCheckFoldsFloatBitsAndIEEEComparisons(t *testing.T) {
	...
	checked, err := Check(parseProgram(t, "let sum: Float32 = 1.5 + 2.25 let negativeZero: Float64 = -0.0 let negatedZero: Float64 = -(-0.0) let nan: Float64 = 0.0 / 0.0 let nanEqual: Bool = (0.0 / 0.0) == (0.0 / 0.0) let nanDifferent: Bool = (0.0 / 0.0) != (0.0 / 0.0)"))
	if err != nil {
		t.Fatalf("Check returned an error: %v", err)
	}
...
func TestCheckRejectsStaticZeroDivisorsIncludingFoldedExpressions(t *testing.T) {
	for _, source := range []string{
		"let result: Int32 = 1 / 0",
		"let mut total: Int32 = 10 let bad: Int32 = total / 0",
		"let mut total: Int32 = 10 let bad: Int32 = total % 0",
		"let mut total: Int32 = 10 let bad: Int32 = total / (2 - 2)",
	} {
...
func TestCheckFoldsStaticSignedMinimumDivisors(t *testing.T) {
	...
	checked, err := Check(parseProgram(t, "let quotient: Int8 = -128 / -1"))
	if err != nil {
	...
	checked, err = Check(parseProgram(t, "let remainder: Int8 = -128 % -1"))
	if err != nil {
...
func TestCheckShortCircuitReachability(t *testing.T) {
	for _, testCase := range []struct {
		...
		{"let result: Bool = true or ((1 / 0) == 0)", true},
		{"let result: Bool = false and ((1 / 0) == 0)", false},
		// Mixed-type logical operands fold through their
		// truthiness; the unreachable RHS never evaluates.
		{"let result: Bool = true or (1 and 2)", true},
		{"let result: Bool = false and (1 or 2)", false},
	} {
	...
	for _, source := range []string{
		"let mut guard: Bool = true let result: Bool = guard or ((1 / 0) == 0)",
		"let mut guard: Bool = false let result: Bool = guard and ((1 / 0) == 0)",
	} {
		_, err := Check(parseProgram(t, source))
		if err == nil || !strings.Contains(err.Error(), "division by zero") {
...
func TestCheckAcceptsUnknownRuntimeDivisors(t *testing.T) {
	if _, err := Check(parseProgram(t, "let mut divisor: Int32 = 0 let total: Int32 = 10 let quotient: Int32 = total / divisor let remainder: Int32 = total % divisor")); err != nil {
...
func TestCheckLogicalOperandsAcceptMixedTypes(t *testing.T) {
	checked, err := Check(parseProgram(t, "let mut count: Int32 = 1 let ready: Bool = true let flag: Bool = count and ready"))
	if err != nil {
	...
	for _, testCase := range []struct {
		...
		{"let flag: Bool = 1 and 2", true},
		{"let flag: Bool = 1 and nil", false},
		{"let flag: Bool = nil or 2", true},
		{"let flag: Bool = !0", false},
		{"let flag: Bool = !nil", true},
	} {
		checked, err := Check(parseProgram(t, testCase.source))
		if err != nil {
...
func TestCheckLogicalNotAcceptsNonBoolOperand(t *testing.T) {
	checked, err := Check(parseProgram(t, "let mut count: Int32 = 1 let flag: Bool = !count"))
	if err != nil {
