...
func TestCheckFoldsFloat64TwoToSixtyThreeToUInt64(t *testing.T) {
	checked := requireAccepted(t, "let big: UInt64 = 9223372036854775808.0.to<UInt64>()")
...
func TestCheckRejectsFloat64TwoToSixtyFourForUInt64(t *testing.T) {
	requireDiagnostic(t, "let bad: UInt64 = 18446744073709551616.0.to<UInt64>()", "value 18446744073709551616 is outside the range of UInt64")
}

func TestCheckRejectsFloat64TwoToSixtyThreeForInt64(t *testing.T) {
	requireDiagnostic(t, "let bad: Int64 = 9223372036854775808.0.to<Int64>()", "value 9223372036854775808 is outside the range of Int64")
}
...
func TestCheckFoldsFloat32ConversionFromRoundedBits(t *testing.T) {
	...
	checked := requireAccepted(t, "let value: UInt32 = 16777217.5.to<Float32>().to<UInt32>()")
	declaration := checked.Statements[0].(Declaration)
...
func TestCheckFoldsFloat64ExactValueBeforeTruncation(t *testing.T) {
	...
	// exact Float64 value.
	checked := requireAccepted(t, "let value: UInt32 = 16777217.5.to<UInt32>()")
	declaration := checked.Statements[0].(Declaration)
...
func TestCheckFoldsIntegerToFloat32RoundingForChainedConversion(t *testing.T) {
	...
	checked := requireAccepted(t, "let value: Int32 = 16777217.0.to<Float32>().to<Int32>()")
	declaration := checked.Statements[0].(Declaration)
...
func TestCheckFoldsNegativeFractionToUnsignedZero(t *testing.T) {
	...
	checked := requireAccepted(t, "let zero: UInt32 = (-0.5).to<UInt32>()")
	declaration := checked.Statements[0].(Declaration)
...
func TestCheckFoldsNegativeFractionForSignedDestination(t *testing.T) {
	checked := requireAccepted(t, "let zero: Int32 = (-0.5).to<Int32>()")
	declaration := checked.Statements[0].(Declaration)
	...
	checked = requireAccepted(t, "let negative: Int32 = (-1.5).to<Int32>()")
	declaration = checked.Statements[0].(Declaration)
...
func TestCheckRejectsTruncatedNegativeForUnsigned(t *testing.T) {
	requireDiagnostic(t, "let bad: UInt32 = (-1.5).to<UInt32>()", "value -1 is outside the range of UInt32")
}

func TestCheckFoldsPositiveFractionTowardZero(t *testing.T) {
	checked := requireAccepted(t, "let value: Int32 = 2.5.to<Int32>()")
	declaration := checked.Statements[0].(Declaration)
...
func TestCheckRejectsNonFiniteAndHugeFloatConversion(t *testing.T) {
	...
	requireDiagnostic(t, "let bad: Int32 = 3.5e38.to<Float32>().to<Int32>()", "floating value cannot be converted to Int32")
	requireDiagnostic(t, "let bad: Int32 = 3.5e38.to<Int32>()", "value 350000000000000001565567347835409530880 is outside the range of Int32")
}
...
func TestCheckFoldsFloatToSizeKeepingTargetDependence(t *testing.T) {
	...
	checked := requireAccepted(t, "let size: Size = 100.0.to<Size>()")
	declaration := checked.Statements[0].(Declaration)
	...
	checked = requireAccepted(t, "let size: Size = 9223372036854775808.0.to<Size>()")
	declaration = checked.Statements[0].(Declaration)
	if got, ok := constant.Uint64Val(declaration.Source.Constant); !ok || got != uint64(1)<<63 {
		t.Fatalf("folded Size = %v, want 2^63", declaration.Source.Constant)
	}
	requireDiagnostic(t, "let bad: Size = 18446744073709551616.0.to<Size>()", "value 18446744073709551616 is outside the range of Size")
}
...
func TestCheckConversionOfKnownBindingReadStaysDynamic(t *testing.T) {
	...
	checked := requireAccepted(t, "let value: Float64 = 2.0 let converted: Int32 = value.to<Int32>()")
