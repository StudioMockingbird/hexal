...
func TestArrayMutableElementWrite(t *testing.T) {
	result := Compile("mut fixed: Array<Int32, 2> = [1, 2] fixed[0] = 7 fixed[1] = fixed[0] + 1")
	if result.ExitCode != ExitSuccess {
		t.Fatalf("Compile exit code = %d (%v), want %d", result.ExitCode, result.Stderr, ExitSuccess)
	}
	for _, want := range []string{
...
func TestArrayConstantIndexOutOfBoundsIsACompileError(t *testing.T) {
	for _, source := range []string{
		...
		if result.ExitCode != ExitFailure || len(result.Stderr) != 1 || !strings.Contains(result.Stderr[0], "out of bounds") {
...
func TestNestedArrays(t *testing.T) {
	result := Compile("grid: Array<Array<Int32, 2>, 2> = [[1, 2], [3, 4]] corner: Int32 = grid[1][0]")
	if result.ExitCode != ExitSuccess {
		t.Fatalf("Compile exit code = %d (%v), want %d", result.ExitCode, result.Stderr, ExitSuccess)
	}
	for _, want := range []string{
...
func TestArrayTrailingCommaLiteral(t *testing.T) {
	result := Compile("fixed: Array<Int32, 3> = [10, 20, 30, ]")
	if result.ExitCode != ExitSuccess {
		t.Fatalf("Compile exit code = %d (%v), want %d", result.ExitCode, result.Stderr, ExitSuccess)
	}
