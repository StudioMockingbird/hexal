...
func TestWrapHeaderRendersSelectedHelpersInDiscoveryOrder(t *testing.T) {
	...
	artifacts, err := renderComponentArtifacts(&programEmission{wrapState: state}, Config{SourceTable: testSpanTable})
	if err != nil {
...
func TestWrapHelpersLeaveHexalHeader(t *testing.T) {
	program := checkedGeneratorSource(t, "let mut value: Int8 = 127 let wrapped: Int8 = value + 1\n")
...
func TestWrapComponentAbsentWithoutSelection(t *testing.T) {
	program := checkedGeneratorSource(t, "let x: Int32 = 13\n")
	files := generateOne(t, program)
	if _, exists := files["hexal/wrap.h"]; exists {
		t.Fatalf("scalar-only program emitted hexal/wrap.h: %v", files)
	}
	artifacts, err := renderComponentArtifacts(&programEmission{wrapState: &generatedWrapState{seen: make(map[string]bool)}}, Config{SourceTable: testSpanTable})
	if err != nil {
		t.Fatalf("renderComponentArtifacts() error = %v", err)
	}
