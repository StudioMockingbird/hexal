...
func TestGenerateTryExpressionNormalizesSuccess(t *testing.T) {
	...
	files, err = GenerateChecked(appModuleGraph(), map[string]checker.Program{"app.hex": multiple}, Config{SourceTable: testSpanTable})
...
func TestGenerateTryInNestedBlockEmitsSinglePrologue(t *testing.T) {
	...
	if !strings.Contains(rootC[ifIdx:], "const hex_t_Int32_Error hex_try_1 = hex_f_m3_app_mk();") {
