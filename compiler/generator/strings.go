...
func stringLiteralCName(index int) string {
	return fmt.Sprintf("sw_lit_%d", index)
}

// writeStringDefinitions emits the sw_string handle and storage typedefs, one
// read-only static object pair per unique literal, and the runtime helpers.
// The byte-view helpers require sw_view_UInt8, which GenerateChecked ensures
...
func writeStringDefinitions(result *strings.Builder, strings *generatedStringState) {
	...
	result.WriteString("typedef struct sw_string_storage {\n    sw_string header;\n    uint8_t bytes[];\n} sw_string_storage;\n")
	if strings.needStrand {
		...
		result.WriteString("typedef struct sw_strand {\n    uint8_t data[32];\n} sw_strand;\n")
	}
	for index, payload := range strings.literals {
		...
		fmt.Fprintf(result, "static const sw_string %s = { %s_bytes, %d };\n", name, name, len(payload))
	}
	result.WriteString("\nstatic inline uint64_t sw_utf8_next(const uint8_t *data, size_t length, size_t *index) {\n")
	...
	result.WriteString("\nstatic inline uint32_t sw_utf8_decode(const uint8_t *data, size_t length, size_t *index) {\n")
	result.WriteString("    size_t start = *index;\n")
	result.WriteString("    sw_utf8_next(data, length, index);\n")
	...
	result.WriteString("}\n")
	result.WriteString("\nstatic inline size_t sw_utf8_encode(uint8_t *out, uint32_t value) {\n")
	...
	result.WriteString("\nstatic inline const sw_string *sw_string_from_bytes(sw_heap h, const uint8_t *data, size_t length) {\n")
	...
	result.WriteString("        sw_utf8_next(data, length, &index);\n")
	result.WriteString("    }\n")
	result.WriteString("    sw_string_storage *storage = sw_heap_raw_allocate(h.identity, sizeof(sw_string_storage) + length + 1, _Alignof(sw_string_storage));\n")
	...
	result.WriteString("\nstatic inline const sw_string *sw_string_from_runes(sw_heap h, const uint32_t *data, size_t length) {\n")
	...
	result.WriteString("    }\n")
	result.WriteString("    sw_string_storage *storage = sw_heap_raw_allocate(h.identity, sizeof(sw_string_storage) + bytes + 1, _Alignof(sw_string_storage));\n")
	result.WriteString("    size_t out = 0;\n")
	result.WriteString("    for (size_t index = 0; index < length; index++) {\n")
	result.WriteString("        out += sw_utf8_encode(storage->bytes + out, data[index]);\n")
	result.WriteString("    }\n")
	result.WriteString("    storage->header.data = storage->bytes;\n")
	result.WriteString("    storage->header.byte_length = bytes;\n")
	result.WriteString("    storage->bytes[bytes] = 0;\n")
	result.WriteString("    return &storage->header;\n}\n")
	result.WriteString("\nstatic inline const sw_string *sw_string_to_string(sw_heap h, const sw_string *text) {\n")
	result.WriteString("    return sw_string_from_bytes(h, text->data, text->byte_length);\n}\n")
	result.WriteString("\nstatic inline const sw_string *sw_string_concat(sw_heap h, const sw_string *left, const sw_string *right) {\n")
	...
	result.WriteString("    storage->bytes[length] = 0;\n")
	result.WriteString("    return &storage->header;\n}\n")
	result.WriteString("\nstatic inline void sw_string_free(sw_heap h, const sw_string *text) {\n")
	...
	result.WriteString("    sw_heap_header *header = (sw_heap_header *)((unsigned char *)text - offset);\n")
	...
	result.WriteString("\nstatic inline sw_view_UInt8 sw_string_bytes(const sw_string *text) {\n")
	result.WriteString("    return (sw_view_UInt8){ text->data, text->byte_length };\n}\n")
	result.WriteString("\nstatic inline sw_view_UInt8 sw_string_slice(const sw_string *text, size_t start, size_t end) {\n")
	result.WriteString("    size_t index = 0;\n")
	result.WriteString("    size_t runes = 0;\n")
	result.WriteString("    while (index < text->byte_length) {\n")
	result.WriteString("        sw_utf8_next(text->data, text->byte_length, &index);\n")
	...
	result.WriteString("    return (sw_view_UInt8){ text->data + byteStart, byteEnd - byteStart };\n}\n")
	result.WriteString("\nstatic inline size_t sw_string_rune_length(const sw_string *text) {\n")
	result.WriteString("    size_t index = 0;\n    size_t runes = 0;\n")
	result.WriteString("    while (index < text->byte_length) {\n")
	result.WriteString("        sw_utf8_next(text->data, text->byte_length, &index);\n")
	result.WriteString("        runes++;\n")
	...
	result.WriteString("\nstatic inline bool sw_string_is_empty(const sw_string *text) {\n")
	result.WriteString("    return text->byte_length == 0;\n}\n")
	result.WriteString("\nstatic inline uint32_t sw_string_at_rune(const sw_string *text, size_t rune_index) {\n")
	result.WriteString("    size_t index = 0;\n    size_t rune = 0;\n")
	...
	result.WriteString("            return sw_utf8_decode(text->data, text->byte_length, &index);\n")
	result.WriteString("        }\n")
	result.WriteString("        sw_utf8_next(text->data, text->byte_length, &index);\n")
	result.WriteString("        rune++;\n")
	result.WriteString("    }\n}\n")
	result.WriteString("\ntypedef struct sw_rune_cursor {\n    const uint8_t *data;\n    size_t length;\n    size_t offset;\n} sw_rune_cursor;\n")
	result.WriteString("\nstatic inline sw_rune_cursor sw_string_rune_cursor(const sw_string *text) {\n")
	result.WriteString("    return (sw_rune_cursor){ text->data, text->byte_length, 0 };\n}\n")
	result.WriteString("\nstatic inline bool sw_rune_cursor_has_next(sw_rune_cursor cursor) {\n")
	result.WriteString("    return cursor.offset < cursor.length;\n}\n")
	result.WriteString("\nstatic inline uint32_t sw_rune_cursor_next(sw_rune_cursor *cursor) {\n")
	result.WriteString("    if (cursor->offset >= cursor->length) {\n")
	result.WriteString("        fputs(\"[Runtime Error] RuneCursor has no next value\\n\", stderr);\n            abort();\n")
	result.WriteString("    }\n")
	result.WriteString("    return sw_utf8_decode(cursor->data, cursor->length, &cursor->offset);\n}\n")
	if strings.needStrand {
		result.WriteString("\nstatic inline size_t sw_strand_rune_length(sw_strand text) {\n")
		result.WriteString("    size_t index = 0;\n    size_t runes = 0;\n")
		result.WriteString("    while (index < 32 && text.data[index] != 0) {\n")
		result.WriteString("        sw_utf8_next(text.data, 32, &index);\n")
		result.WriteString("        runes++;\n")
		result.WriteString("    }\n")
		result.WriteString("    return runes;\n}\n")
		result.WriteString("\nstatic inline bool sw_strand_is_empty(sw_strand text) {\n")
		result.WriteString("    return text.data[0] == 0;\n}\n")
		result.WriteString("\nstatic inline uint32_t sw_strand_at_rune(sw_strand text, size_t rune_index) {\n")
		result.WriteString("    size_t index = 0;\n    size_t rune = 0;\n")
		result.WriteString("    for (;;) {\n")
		...
		result.WriteString("        }\n")
		result.WriteString("        if (rune == rune_index) {\n")
		result.WriteString("            return sw_utf8_decode(text.data, 32, &index);\n")
		result.WriteString("        }\n")
		result.WriteString("        sw_utf8_next(text.data, 32, &index);\n")
		result.WriteString("        rune++;\n")
		result.WriteString("    }\n}\n")
		result.WriteString("\nstatic inline const sw_string *sw_strand_to_string(sw_heap h, sw_strand text) {\n")
		result.WriteString("    size_t length = 0;\n")
		result.WriteString("    while (length < 32 && text.data[length] != 0) {\n")
		result.WriteString("        length++;\n")
		result.WriteString("    }\n")
		result.WriteString("    return sw_string_from_bytes(h, text.data, length);\n}\n")
	}
}
