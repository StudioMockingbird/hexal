...
func writeStringDefinitions(result *strings.Builder, stringState *generatedStringState) {
	...
	result.WriteString("        hex_runtime_trap(\"[Runtime Error] invalid UTF-8 in String\\n\");\n    }\n")
	...
	result.WriteString("            hex_runtime_trap(\"[Runtime Error] invalid Unicode scalar value\\n\");\n")
	result.WriteString("        }\n")
	result.WriteString("        size_t width;\n")
	result.WriteString("        if (value < 0x80) {\n")
	result.WriteString("            width = 1;\n")
	...
	result.WriteString("            width = 4;\n")
	result.WriteString("        }\n")
	result.WriteString("        if (ckd_add(&bytes, bytes, width)) {\n")
	result.WriteString("            hex_runtime_trap(\"[Runtime Error] string allocation size overflow\\n\");\n")
	result.WriteString("        }\n")
	result.WriteString("    }\n")
	result.WriteString("    // The header, payload, and terminator chain is checked with ckd_add\n")
	...
	result.WriteString("    }\n")
	result.WriteString("    if (right->byte_length != 0) {\n")
	result.WriteString("        memcpy(storage->bytes + left->byte_length, right->data, right->byte_length);\n")
	result.WriteString("    }\n")
	result.WriteString("    storage->bytes[length] = 0;\n")
	result.WriteString("    return &storage->header;\n}\n")
	result.WriteString("\nstatic inline void hex_string_free(hex_heap h, const hex_string *text) {\n")
	result.WriteString("    if (text == nullptr) {\n")
	...
	result.WriteString("        hex_runtime_trap(\"[Runtime Error] string slice bounds out of range\\n\");\n    }\n")
	...
	result.WriteString("            hex_runtime_trap(\"[Runtime Error] String index is outside its bounds\\n\");\n")
	result.WriteString("        }\n")
	...
	result.WriteString("        hex_runtime_trap(\"[Runtime Error] RuneCursor has no next value\\n\");\n")
	result.WriteString("    }\n")
