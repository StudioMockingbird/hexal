...
func TestGenerateHeapAllocationAndFree(t *testing.T) {
	program := checkedGeneratorSource(t, "let h: Heap = Heap() let p: Ptr<mut Int32> = h.allocate<Int32>(0) defer h.free(p)")
...
func TestGenerateHeapHandleSelectsComponentPair(t *testing.T) {
	program := checkedGeneratorSource(t, "let h: Heap = Heap()\n")
	files := generateOne(t, program)
...
func TestGenerateDeferReverseOrderAndCapture(t *testing.T) {
	program := checkedGeneratorSource(t, "fun record(value: Int32) do end let mut first: Int32 = 1 let mut second: Int32 = 2 defer record(first) defer record(second)")
