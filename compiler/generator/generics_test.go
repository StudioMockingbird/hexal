...
func TestGenerateGenericFunctionSpecialization(t *testing.T) {
	...
	if !strings.Contains(rootC, "hex_f_identity_Int32") {
		t.Fatalf("main.c = %q, want specialized function name", rootC)
	}
	if strings.Count(rootC, "hex_f_identity_Int32") < 2 {
...
func TestGenerateGenericObjectSpecialization(t *testing.T) {
	...
	if !strings.Contains(rootH, "hex_t_Box_Int32") {
...
func TestGenerateGenericMethodSpecialization(t *testing.T) {
	...
	if !strings.Contains(rootC, "hex_f_Box_Int32__get") && !strings.Contains(rootC, "hex_f_Box_Int32_get") {
