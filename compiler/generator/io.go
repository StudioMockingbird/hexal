...
func writeIOPrelude(result *strings.Builder, state *generatedIOState) {
	...
	result.WriteString("typedef enum sw_file_mode {\n    SW_FILE_READ,\n    SW_FILE_WRITE,\n    SW_FILE_APPEND,\n} sw_file_mode;\n")
	result.WriteString("typedef struct sw_file {\n    FILE *stream;\n    sw_file_mode mode;\n    bool owned;\n} sw_file;\n")
...
func writeIODefinitions(result *strings.Builder, state *generatedIOState, strings *generatedStringState, schedulerLinked bool) {
	...
	if schedulerLinked {
		result.WriteString("\nstatic mtx_t sw_io_gate;\nstatic bool sw_io_gate_closed;\n")
		result.WriteString("\nstatic void sw_io_gate_lock(void) {\n    mtx_lock(&sw_io_gate);\n}\n")
		result.WriteString("static void sw_io_gate_unlock(void) {\n    mtx_unlock(&sw_io_gate);\n}\n")
		result.WriteString("\nstatic void sw_io_gate_shutdown(void) {\n    mtx_lock(&sw_io_gate);\n    sw_io_gate_closed = true;\n    fflush(stdout);\n    fflush(stderr);\n    mtx_unlock(&sw_io_gate);\n}\n")
	} else {
		result.WriteString("\nstatic bool sw_io_gate_closed;\n")
		result.WriteString("static void sw_io_gate_lock(void) {\n}\n")
		result.WriteString("static void sw_io_gate_unlock(void) {\n}\n")
		result.WriteString("\nstatic void sw_io_gate_shutdown(void) {\n    fflush(stdout);\n    fflush(stderr);\n}\n")
	}
	state.writeIOErrorHelper(result)
	result.WriteString("\nstatic inline bool sw_utf8_valid(const uint8_t *data, size_t length) {\n    size_t index = 0;\n    while (index < length) {\n        uint8_t lead = data[index];\n        size_t width;\n        if (lead < 0x80) {\n            width = 1;\n        } else if (lead < 0xE0) {\n            width = 2;\n        } else if (lead < 0xF0) {\n            width = 3;\n        } else if (lead < 0xF8) {\n            width = 4;\n        } else {\n            return false;\n        }\n        if (index + width > length) {\n            return false;\n        }\n        for (size_t continuation = 1; continuation < width; continuation++) {\n            if ((data[index + continuation] & 0xC0) != 0x80) {\n                return false;\n            }\n        }\n        index += width;\n    }\n    return true;\n}\n")
	result.WriteString("\nstatic inline bool sw_file_path_valid(const uint8_t *data, size_t length) {\n    if (length == 0) {\n        return false;\n    }\n    for (size_t index = 0; index < length; index++) {\n        if (data[index] == 0) {\n            return false;\n        }\n        if (data[index] > 0x7F) {\n            return false;\n        }\n    }\n    return true;\n}\n")
	if state.open {
		...
		if union != (compilerTypes.Type{}) {
			...
			fmt.Fprintf(result, "\nstatic inline %s sw_file_open(const uint8_t *path, size_t length, sw_file_mode mode, size_t line, size_t column, const sw_string *message) {\n    (void)message;\n    if (!sw_file_path_valid(path, length)) {\n        return (%s){ .tag = %s, .payload.member_%d = sw_io_error(line, column, &%s) };\n    }\n    FILE *stream = fopen((const char *)path, mode == SW_FILE_READ ? \"rb\" : (mode == SW_FILE_WRITE ? \"wb\" : \"ab\"));\n    if (stream == NULL) {\n        return (%s){ .tag = %s, .payload.member_%d = sw_io_error(line, column, &%s) };\n    }\n    return (%s){ .tag = %s, .payload.member_%d = (sw_file){ stream, mode, true } };\n}\n",
	...
	if state.readBytes {
		...
		if union != (compilerTypes.Type{}) && listType.List != nil {
			...
			fmt.Fprintf(result, "\nstatic inline %s sw_file_read_bytes_%s(sw_file file, sw_heap h, size_t line, size_t column, const sw_string *message) {\n    (void)message;\n    if (file.mode == SW_FILE_WRITE || file.mode == SW_FILE_APPEND) {\n        return (%s){ .tag = %s, .payload.member_%d = sw_io_error(line, column, &%s) };\n    }\n    %s *list = sw_list_new_%s(h);\n    uint8_t buffer[4096];\n    for (;;) {\n        size_t count = fread(buffer, 1, sizeof buffer, file.stream);\n        for (size_t index = 0; index < count; index++) {\n            sw_list_push_%s(list, buffer[index]);\n        }\n        if (count < sizeof buffer) {\n            if (ferror(file.stream)) {\n                sw_list_free_%s(h, list);\n                return (%s){ .tag = %s, .payload.member_%d = sw_io_error(line, column, &%s) };\n            }\n            break;\n        }\n    }\n    return (%s){ .tag = %s, .payload.member_%d = list };\n}\n",
	...
	if state.write {
		...
		if union != (compilerTypes.Type{}) {
			...
			fmt.Fprintf(result, "\nstatic inline %s sw_file_write_bytes(sw_file file, const uint8_t *data, size_t length, size_t line, size_t column, const sw_string *message, const sw_string *mode_message) {\n    (void)message;\n    (void)mode_message;\n    if (file.mode == SW_FILE_READ) {\n        return (%s){ .tag = %s, .payload.member_%d = sw_io_error(line, column, &%s) };\n    }\n    size_t written = 0;\n    while (written < length) {\n        size_t count = fwrite(data + written, 1, length - written, file.stream);\n        if (count == 0) {\n            return (%s){ .tag = %s, .payload.member_%d = sw_io_error(line, column, &%s) };\n        }\n        written += count;\n    }\n    return (%s){ .tag = %s };\n}\n",
	...
	if state.writeText {
		...
		if union != (compilerTypes.Type{}) {
			...
			message := state.ioMessage(strings, ioCouldNotWrite)
			modeMessage := state.ioMessage(strings, ioModeError)
			fmt.Fprintf(result, "\nstatic inline %s sw_file_write_text(sw_file file, const sw_string *text, size_t line, size_t column, const sw_string *message, const sw_string *mode_message, bool standard_gate) {\n    (void)message;\n    (void)mode_message;\n    if (standard_gate) {\n        sw_io_gate_lock();\n        if (sw_io_gate_closed) {\n            sw_io_gate_unlock();\n            return (%s){ .tag = %s, .payload.member_%d = sw_io_error(line, column, &%s) };\n        }\n    }\n    if (file.mode == SW_FILE_READ) {\n        if (standard_gate) {\n            sw_io_gate_unlock();\n        }\n        return (%s){ .tag = %s, .payload.member_%d = sw_io_error(line, column, &%s) };\n    }\n    size_t written = 0;\n    while (written < text->byte_length) {\n        size_t count = fwrite(text->data + written, 1, text->byte_length - written, file.stream);\n        if (count == 0) {\n            if (standard_gate) {\n                sw_io_gate_unlock();\n            }\n            return (%s){ .tag = %s, .payload.member_%d = sw_io_error(line, column, &%s) };\n        }\n        written += count;\n    }\n    if (standard_gate) {\n        sw_io_gate_unlock();\n    }\n    return (%s){ .tag = %s };\n}\n",
				union.CName, union.CName, unionTagName(union, errorIndex), errorIndex, modeMessage,
	...
	if state.flush {
		...
		if union != (compilerTypes.Type{}) {
			...
			modeMessage := state.ioMessage(strings, ioModeError)
			fmt.Fprintf(result, "\nstatic inline %s sw_file_flush(sw_file file, size_t line, size_t column, const sw_string *message, const sw_string *mode_message, bool standard_gate) {\n    (void)message;\n    (void)mode_message;\n    if (standard_gate) {\n        sw_io_gate_lock();\n        if (sw_io_gate_closed) {\n            sw_io_gate_unlock();\n            return (%s){ .tag = %s, .payload.member_%d = sw_io_error(line, column, &%s) };\n        }\n    }\n    if (file.mode == SW_FILE_READ) {\n        if (standard_gate) {\n            sw_io_gate_unlock();\n        }\n        return (%s){ .tag = %s, .payload.member_%d = sw_io_error(line, column, &%s) };\n    }\n    if (fflush(file.stream) != 0) {\n        if (standard_gate) {\n            sw_io_gate_unlock();\n        }\n        return (%s){ .tag = %s, .payload.member_%d = sw_io_error(line, column, &%s) };\n    }\n    if (standard_gate) {\n        sw_io_gate_unlock();\n    }\n    return (%s){ .tag = %s };\n}\n",
				union.CName, union.CName, unionTagName(union, errorIndex), errorIndex, modeMessage,
				union.CName, unionTagName(union, errorIndex), errorIndex, modeMessage,
				union.CName, unionTagName(union, errorIndex), errorIndex, message,
				union.CName, unionTagName(union, nilIndex))
	...
	if state.close {
		result.WriteString("\nstatic inline void sw_file_close(sw_file file) {\n")
...
func (state *generatedIOState) writeIOErrorHelper(result *strings.Builder) {
	fmt.Fprintf(result, "\nstatic inline sw_t_Error sw_io_error(size_t line, size_t column, const sw_string *message) {\n")
...
func renderFileOpen(node checker.Expression, state *expressionValidation) (string, error) {
	...
	return fmt.Sprintf("sw_file_open((%s)->data, (%s)->byte_length, %s, %d, %d, &%s)", path, path, mode, node.SourceLine, node.SourceColumn, message), nil
}
...
func renderStdioCall(node checker.Expression) (string, error) {
	switch node.Name {
	case "stdin":
		return "((sw_file){ stdin, SW_FILE_READ, false })", nil
	case "stdout":
		return "((sw_file){ stdout, SW_FILE_WRITE, false })", nil
	case "stderr":
		return "((sw_file){ stderr, SW_FILE_WRITE, false })", nil
	}
...
func renderFileMethod(node checker.Expression, state *expressionValidation) (string, error) {
	...
	case "read_bytes":
		...
		return fmt.Sprintf("sw_file_read_bytes_%s(%s, %s, %d, %d, &%s)", listSuffix(listType), receiver, heap, node.SourceLine, node.SourceColumn, message), nil
	case "read_text":
		if len(node.Arguments) != 1 {
		...
		return fmt.Sprintf("sw_file_read_text(%s, %s, %d, %d, &%s, &%s)", receiver, heap, node.SourceLine, node.SourceColumn, message, utf8Message), nil
	case "write":
		if len(node.Arguments) != 1 {
		...
		return fmt.Sprintf("sw_file_write_bytes(%s, (%s).data, (%s).length, %d, %d, &%s, &%s)", receiver, view, view, node.SourceLine, node.SourceColumn, message, modeMessage), nil
	case "write_text":
		if len(node.Arguments) != 1 {
		...
		return fmt.Sprintf("sw_file_write_text(%s, %s, %d, %d, &%s, &%s, %s)", receiver, text, node.SourceLine, node.SourceColumn, message, modeMessage, gate), nil
	case "flush":
		...
		return fmt.Sprintf("sw_file_flush(%s, %d, %d, &%s, &%s, %s)", receiver, node.SourceLine, node.SourceColumn, message, modeMessage, gate), nil
	case "close":
		return "sw_file_close(" + receiver + ")", nil
	}
