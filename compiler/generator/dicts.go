...
func dictSuffix(dict compilerTypes.Type) string {
	return strings.TrimPrefix(dict.CName, "sw_dict_")
}

// writeDictDefinitions emits the sw_strand value type once, plus one entry
...
func writeDictDefinitions(result *strings.Builder, dicts *generatedDictState) {
	...
	for _, dict := range dicts.order {
		...
		entryName := "sw_dict_entry_" + suffix
		...
		if compilerTypes.IsStrand(key) {
			...
			fmt.Fprintf(result, "static inline bool sw_dict_key_equal_%s(sw_strand left, sw_strand right) {\n    for (size_t index = 0; index < 32; index++) {\n        if (left.data[index] != right.data[index]) {\n            return false;\n        }\n    }\n    return true;\n}\n", suffix)
		} else {
			hashInt32Written = writeInt32HashHelper(result, hashInt32Written)
		}
		fmt.Fprintf(result, "static inline uint64_t sw_dict_probe_%s_region(%s *region, uint64_t capacity, %s key) {\n", suffix, entryName, keySpelling)
		if compilerTypes.IsStrand(key) {
			result.WriteString("    uint64_t hash = sw_hash_Strand(key);\n")
		} else {
			result.WriteString("    uint64_t hash = sw_hash_Int32(key);\n")
		}
		result.WriteString("    size_t index = hash & (capacity - 1);\n")
		if compilerTypes.IsStrand(key) {
			fmt.Fprintf(result, "    while (region[index].active && !sw_dict_key_equal_%s(region[index].key, key)) {\n        index = (index + 1) & (capacity - 1);\n    }\n", suffix)
		} else {
			result.WriteString("    while (region[index].active && region[index].key != key) {\n        index = (index + 1) & (capacity - 1);\n    }\n")
		}
		result.WriteString("    return index;\n}\n")
		fmt.Fprintf(result, "static inline uint64_t sw_dict_probe_%s(const %s *dict, %s key) {\n", suffix, dict.CName, keySpelling)
		if compilerTypes.IsStrand(key) {
			result.WriteString("    uint64_t hash = sw_hash_Strand(key);\n")
		} else {
			result.WriteString("    uint64_t hash = sw_hash_Int32(key);\n")
		...
		if compilerTypes.IsStrand(key) {
			fmt.Fprintf(result, "    while (dict->buckets[index].active && !sw_dict_key_equal_%s(dict->buckets[index].key, key)) {\n        index = (index + 1) & (dict->capacity - 1);\n    }\n", suffix)
		} else {
			result.WriteString("    while (dict->buckets[index].active && dict->buckets[index].key != key) {\n        index = (index + 1) & (dict->capacity - 1);\n    }\n")
		}
		result.WriteString("    return index;\n}\n")
		fmt.Fprintf(result, "static inline %s *sw_dict_new_%s(sw_heap h) {\n", dict.CName, suffix)
		result.WriteString("    " + dict.CName + " *header = sw_heap_raw_allocate(h.identity, sizeof(" + dict.CName + "), _Alignof(" + dict.CName + "));\n")
		result.WriteString("    header->buckets = NULL;\n    header->length = 0;\n    header->capacity = 0;\n    header->allocator = h.identity;\n")
		fmt.Fprintf(result, "    return header;\n}\n")
		fmt.Fprintf(result, "static inline void sw_dict_grow_%s(%s *dict) {\n", suffix, dict.CName)
		...
		result.WriteString("    " + entryName + " *region = sw_heap_raw_allocate(dict->allocator, next * sizeof(" + entryName + "), _Alignof(" + entryName + "));\n")
		result.WriteString("    for (size_t index = 0; index < next; index++) {\n        region[index].active = false;\n    }\n")
		fmt.Fprintf(result, "    for (size_t index = 0; index < dict->capacity; index++) {\n        if (dict->buckets[index].active) {\n            uint64_t probe = sw_dict_probe_%s_region(region, next, dict->buckets[index].key);\n            region[probe] = dict->buckets[index];\n        }\n    }\n", suffix)
		result.WriteString("    free(dict->buckets);\n    dict->buckets = region;\n    dict->capacity = next;\n}\n")
		fmt.Fprintf(result, "static inline void sw_dict_insert_%s(%s *dict, %s key, %s value) {\n", suffix, dict.CName, keySpelling, valueSpelling)
		result.WriteString("    if (dict->capacity == 0 || (dict->length + 1) * 10 >= dict->capacity * 7) {\n        sw_dict_grow_" + suffix + "(dict);\n    }\n")
		result.WriteString("    size_t index = sw_dict_probe_" + suffix + "(dict, key);\n")
		if stringValue {
			result.WriteString("    const sw_string *copy = sw_string_from_bytes((sw_heap){ dict->allocator }, value->data, value->byte_length);\n")
		}
		fmt.Fprintf(result, "    if (dict->buckets[index].active) {\n")
		if stringValue {
			result.WriteString("        sw_string_free((sw_heap){ dict->allocator }, dict->buckets[index].value);\n")
		...
		fmt.Fprintf(result, "static inline %s sw_dict_get_%s(const %s *dict, %s key) {\n", valueSpelling, suffix, dict.CName, keySpelling)
		...
		fmt.Fprintf(result, "static inline bool sw_dict_contains_%s(const %s *dict, %s key) {\n", suffix, dict.CName, keySpelling)
		fmt.Fprintf(result, "    if (dict->capacity == 0) {\n        return false;\n    }\n")
		result.WriteString("    size_t index = sw_dict_probe_" + suffix + "(dict, key);\n")
		result.WriteString("    return dict->buckets[index].active;\n}\n")
		fmt.Fprintf(result, "static inline %s sw_dict_remove_%s(%s *dict, %s key) {\n", valueSpelling, suffix, dict.CName, keySpelling)
		fmt.Fprintf(result, "    if (dict->capacity == 0) {\n        fputs(\"[Runtime Error] dictionary key not found\\n\", stderr);\n        abort();\n    }\n")
		result.WriteString("    size_t index = sw_dict_probe_" + suffix + "(dict, key);\n")
		fmt.Fprintf(result, "    if (!dict->buckets[index].active) {\n        fputs(\"[Runtime Error] dictionary key not found\\n\", stderr);\n        abort();\n    }\n")
		...
		fmt.Fprintf(result, "static inline void sw_dict_free_%s(sw_heap h, %s *dict) {\n", suffix, dict.CName)
...
func writeInt32HashHelper(result *strings.Builder, written bool) bool {
	...
	result.WriteString("\nstatic inline uint64_t sw_hash_Int32(int32_t key) {\n")
...
func writeStrandHashHelper(result *strings.Builder, written bool) bool {
	...
	result.WriteString("\nstatic inline uint64_t sw_hash_Strand(sw_strand key) {\n")
