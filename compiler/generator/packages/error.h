...
// find loudly.
static inline {{.MessageType}} hex_error_message(hex_text text) {
    {{.MessageType}} message = { .byte_length = text.length };
...
static inline {{.HeaderType}} hex_error_kind_header(hex_t_ErrorKind kind) {
    switch (kind.tag) {
{{- range .KindVariants}}
    case {{.Tag}}:
        return ({{$.HeaderType}}){ .byte_length = {{.HeaderLength}}, .data = {{.HeaderLiteral}} };
{{- end}}
    case {{.OtherTag}}:
