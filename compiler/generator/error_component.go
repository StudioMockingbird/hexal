...
// errorComponent returns the generated hexal/error.h artifact when Error is
...
func errorComponent(merged *programEmission) ([]componentArtifact, error) {
	if !merged.errorUsed {
		return nil, nil
	}
...
// errorFamilyContent is the drained transition seam: the Error definition
// lives in hexal/error.h, so hexal.h contributes nothing and the emission
// call site receives an empty string.
func errorFamilyContent() string {
	return ""
}
